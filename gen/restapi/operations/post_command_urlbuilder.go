...
func (o *PostCommandURL) Build() (*url.URL, error) {
	...
	var _path = "/command/{command}"

	command := o.Command
	if command != "" {
		_path = strings.Replace(_path, "{command}", command, -1)
	} else {
		return nil, errors.New("command is required on PostCommandURL")
	}
...
func (o *PostCommandURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PostCommandURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PostCommandURL")
	}
